...
class MultistoreCategoryPublishTest extends BaseDirectDbEntityUpdateTest {
    ...
    public function shouldPublishActiveCategories() {
        ...
        $category = $this->insertCategory($parentCategoryId, true, 'Category name');
        $this->setCategoryNameAndStatus($category, 'Category name in first store', false, self::$store1Id);
        $this->setCategoryNameAndStatus($category, 'Category name in second store', true, self::$store2Id);
    ...
    public function shouldPublishCategoriesAssignedToStore() {
        ...
        $store1Category = $this->insertCategory($rootCategoryIdForStore1, true, 'Bikes for first store');
        $store2Category = $this->insertCategory($rootCategoryIdForStore2, true, 'Bikes for second store');
    ...
    private function insertRootCategory(string $name): EntityIds {
        return $this->insertCategory(1, true, $name);
    }

    private function insertCategory(int $parentCategoryId, bool $active, string $name): EntityIds {
        $rootCategoryId = 1;

        $displayModeAttrId = self::attrId('display_mode');
        $includeInMenuAttrId = self::attrId('include_in_menu');

        $rootPath = $parentCategoryId === $rootCategoryId ? "$rootCategoryId" : "$rootCategoryId/$parentCategoryId";
        $category = self::$db->insertCategory($parentCategoryId, $rootPath);

        self::$db->insertVarcharCategoryAttribute($category, $displayModeAttrId, 'PRODUCTS');
        self::$db->insertIntCategoryAttribute($category, $includeInMenuAttrId, 1);
        $this->setCategoryNameAndStatus($category, $name, $active);

        return $category;
    }

    private function setCategoryNameAndStatus(EntityIds $category, string $name, bool $isActive, int $storeId = self::DEFAULT_STORE_ID): void {
        $urlKey = strtolower(str_replace(' ', '_', $name));
        $activeStatus = $isActive ? 1 : 0;

        self::$db->insertVarcharCategoryAttribute($category, self::attrId('name'), $name, $storeId);
        self::$db->insertVarcharCategoryAttribute($category, self::attrId('url_key'), $urlKey, $storeId);
        self::$db->insertIntCategoryAttribute($category, self::attrId('is_active'), $activeStatus, $storeId);
    }
